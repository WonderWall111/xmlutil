...
public actual class XBitSet actual constructor(size: Int) {
    ...
    actual fun isEmpty(): Boolean = bits.all { it == ALL_FALSE }
    ...
    private inline fun doOperation(another: XBitSet, operation: Long.(Long) -> Long) {
    ...
    actual fun andNot(another: XBitSet) {
        ensureCapacity(another.lastIndex)
        var index = 0
        while (index < another.bits.size) {
    ...
    actual fun intersects(another: XBitSet): Boolean =
...
internal actual val XBitSet.isEmpty: Boolean
